    ...
    describe('streamAggregated', () => {
        describe('when streaming a simple text message', () => {
            ...
            it('throws MaxTokenError when stopReason is MaxTokenError', async () => {
                ...
                const messages = [{ type: 'message', role: 'user', content: [{ type: 'textBlock', text: 'Hi' }] }];
                await expect(async () => await collectGenerator(provider.streamAggregated(messages))).rejects.toThrow('Model reached maximum token limit. This is an unrecoverable state that requires intervention.');
            });
        });
        ...
        describe('when streaming tool use', () => {
            ...
            it('throws MaxTokenError when stopReason is MaxTokenError and toolUse is partial', async () => {
                ...
                const messages = [{ type: 'message', role: 'user', content: [{ type: 'textBlock', text: 'Hi' }] }];
                await expect(async () => await collectGenerator(provider.streamAggregated(messages))).rejects.toThrow(MaxTokensError);
            });
        });
