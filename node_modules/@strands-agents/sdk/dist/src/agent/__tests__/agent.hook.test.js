...
import { AfterInvocationEvent, AfterModelCallEvent, AfterToolCallEvent, BeforeInvocationEvent, BeforeModelCallEvent, BeforeToolCallEvent, MessageAddedEvent, ModelStreamEventHook, } from '../../hooks/index.js';
...
describe('Agent Hooks Integration', () => {
    ...
    describe('invocation lifecycle', () => {
        it('fires hooks during invoke', async () => {
            ...
            expect(lifecycleProvider.invocations[1]).toEqual(new MessageAddedEvent({ agent: agent, message: new Message({ role: 'user', content: [new TextBlock('Hi')] }) }));
            expect(lifecycleProvider.invocations[2]).toEqual(new BeforeModelCallEvent({ agent: agent }));
            expect(lifecycleProvider.invocations[3]).toEqual(new AfterModelCallEvent({
                agent,
                stopData: {
                    stopReason: 'endTurn',
            ...
            expect(lifecycleProvider.invocations[4]).toEqual(new MessageAddedEvent({
                agent,
                message: new Message({ role: 'assistant', content: [new TextBlock('Hello')] }),
            }));
        ...
        it('fires hooks during stream', async () => {
            ...
            expect(lifecycleProvider.invocations).toHaveLength(6);
            expect(lifecycleProvider.invocations[0]).toEqual(new BeforeInvocationEvent({ agent: agent }));
            expect(lifecycleProvider.invocations[1]).toEqual(new MessageAddedEvent({
                agent: agent,
    ...
    describe('multi-turn conversations', () => {
        it('fires hooks for each invoke call', async () => {
            ...
            const invocationEvents = lifecycleProvider.invocations.filter((e) => e instanceof BeforeInvocationEvent || e instanceof AfterInvocationEvent);
    ...
    describe('tool execution hooks', () => {
        it('fires tool hooks during tool execution', async () => {
            ...
            const beforeToolCall = beforeToolCallEvents[0];
            expect(beforeToolCall).toEqual(new BeforeToolCallEvent({
                agent,
            ...
            expect(afterToolCall).toEqual(new AfterToolCallEvent({
                agent,
                toolUse: { name: 'testTool', toolUseId: 'tool-1', input: {} },
                tool,
    ...
    describe('MessageAddedEvent', () => {
        it('fires for initial user input', async () => {
            const initialMessage = { role: 'user', content: [{ type: 'textBlock', text: 'Initial' }] };
