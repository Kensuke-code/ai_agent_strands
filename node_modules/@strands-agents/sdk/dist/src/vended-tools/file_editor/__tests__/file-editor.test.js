...
describe('fileEditor tool', () => {
    ...
    const createTestDirectory = async (dirName, files) => {
    ...
    describe('view command', () => {
        ...
        describe('error cases', () => {
            it('throws when file not found', async () => {
                ...
                await expect(fileEditor.invoke({ command: 'view', path: nonExistentPath }, context)).rejects.toThrow('does not exist');
            });
            it('throws when path is not absolute', async () => {
                await expect(fileEditor.invoke({ command: 'view', path: 'relative/path.txt' }, context)).rejects.toThrow('not an absolute path');
            });
            it('throws when view_range has invalid start line', async () => {
                const filePath = await createTestFile('test.txt', 'Line 1\nLine 2\nLine 3');
                await expect(fileEditor.invoke({ command: 'view', path: filePath, view_range: [0, 2] }, context)).rejects.toThrow('view_range');
            });
            it('throws when view_range end is beyond file length', async () => {
                const filePath = await createTestFile('test.txt', 'Line 1\nLine 2\nLine 3');
                await expect(fileEditor.invoke({ command: 'view', path: filePath, view_range: [1, 10] }, context)).rejects.toThrow('view_range');
            });
            it('throws when view_range end is before start', async () => {
                const filePath = await createTestFile('test.txt', 'Line 1\nLine 2\nLine 3');
                await expect(fileEditor.invoke({ command: 'view', path: filePath, view_range: [3, 1] }, context)).rejects.toThrow('view_range');
            });
            it('throws when view_range is provided for directory', async () => {
                ...
                await expect(fileEditor.invoke({ command: 'view', path: dirPath, view_range: [1, 2] }, context)).rejects.toThrow('not allowed when');
            });
        });
    });
    describe('create command', () => {
        ...
        describe('error cases', () => {
            it('throws when file already exists', async () => {
                ...
                await expect(fileEditor.invoke({ command: 'create', path: filePath, file_text: 'new content' }, context)).rejects.toThrow('already exists');
            });
            it('throws when path is not absolute', async () => {
                await expect(fileEditor.invoke({ command: 'create', path: 'relative/path.txt', file_text: 'content' }, context)).rejects.toThrow('not an absolute path');
            });
            it('throws when path contains traversal', async () => {
                const filePath = '..outside.txt';
                await expect(fileEditor.invoke({ command: 'create', path: filePath, file_text: 'content' }, context)).rejects.toThrow();
            });
            it('throws when trying to create in directory as path', async () => {
                const dirPath = await createTestDirectory('testdir', {});
                await expect(fileEditor.invoke({ command: 'create', path: dirPath, file_text: 'content' }, context)).rejects.toThrow('already exists');
            });
        });
    });
    describe('str_replace command', () => {
        it('replaces unique string occurrence', async () => {
            const filePath = await createTestFile('test.txt', 'Line 1\nLine 2 OLD\nLine 3\nLine 4');
            const result = await fileEditor.invoke({ command: 'str_replace', path: filePath, old_str: 'OLD', new_str: 'NEW' }, context);
        ...
        it('handles empty new_str (deletion)', async () => {
            const filePath = await createTestFile('test.txt', 'Line 1\nLine 2 DELETE_ME\nLine 3');
            const result = await fileEditor.invoke({ command: 'str_replace', path: filePath, old_str: ' DELETE_ME', new_str: '' }, context);
            expect(result).toContain('has been edited');
            const fileContent = await fs.readFile(filePath, 'utf-8');
            expect(fileContent).toBe('Line 1\nLine 2\nLine 3');
        });
        it('handles multi-line old_str', async () => {
            const filePath = await createTestFile('test.txt', 'Line 1\nOLD LINE 1\nOLD LINE 2\nLine 4');
            const result = await fileEditor.invoke({ command: 'str_replace', path: filePath, old_str: 'OLD LINE 1\nOLD LINE 2', new_str: 'NEW LINE' }, context);
            expect(result).toContain('has been edited');
            const fileContent = await fs.readFile(filePath, 'utf-8');
            expect(fileContent).toBe('Line 1\nNEW LINE\nLine 4');
        });
        describe('error cases', () => {
            it('throws when old_str not found', async () => {
                const filePath = await createTestFile('test.txt', 'Line 1\nLine 2\nLine 3');
                await expect(fileEditor.invoke({ command: 'str_replace', path: filePath, old_str: 'NOTFOUND', new_str: 'NEW' }, context)).rejects.toThrow('did not appear');
            });
            it('throws when multiple occurrences of old_str', async () => {
                const filePath = await createTestFile('test.txt', 'DUP Line 1\nLine 2\nDUP Line 3');
                await expect(fileEditor.invoke({ command: 'str_replace', path: filePath, old_str: 'DUP', new_str: 'NEW' }, context)).rejects.toThrow('Multiple occurrences');
            });
            it('throws when file not found', async () => {
                const nonExistentPath = path.join(testDir, 'nonexistent.txt');
                await expect(fileEditor.invoke({ command: 'str_replace', path: nonExistentPath, old_str: 'OLD', new_str: 'NEW' }, context)).rejects.toThrow('does not exist');
            });
            it('throws when path is directory', async () => {
                const dirPath = await createTestDirectory('testdir', {});
                await expect(fileEditor.invoke({ command: 'str_replace', path: dirPath, old_str: 'OLD', new_str: 'NEW' }, context)).rejects.toThrow('directory');
            });
        });
    });
    describe('insert command', () => {
        it('inserts at beginning (line 0)', async () => {
            const filePath = await createTestFile('test.txt', 'Line 1\nLine 2\nLine 3');
            const result = await fileEditor.invoke({ command: 'insert', path: filePath, insert_line: 0, new_str: 'NEW LINE' }, context);
            expect(result).toContain('has been edited');
            const fileContent = await fs.readFile(filePath, 'utf-8');
        ...
        it('inserts in middle', async () => {
            const filePath = await createTestFile('test.txt', 'Line 1\nLine 2\nLine 3');
            const result = await fileEditor.invoke({ command: 'insert', path: filePath, insert_line: 2, new_str: 'NEW LINE' }, context);
            expect(result).toContain('has been edited');
            const fileContent = await fs.readFile(filePath, 'utf-8');
            expect(fileContent).toBe('Line 1\nLine 2\nNEW LINE\nLine 3');
        });
        it('inserts at end', async () => {
            const filePath = await createTestFile('test.txt', 'Line 1\nLine 2\nLine 3');
            const result = await fileEditor.invoke({ command: 'insert', path: filePath, insert_line: 3, new_str: 'NEW LINE' }, context);
            expect(result).toContain('has been edited');
            const fileContent = await fs.readFile(filePath, 'utf-8');
            expect(fileContent).toBe('Line 1\nLine 2\nLine 3\nNEW LINE');
        });
        it('shows snippet with 4 lines before and after insertion', async () => {
            const content = 'Line 1\nLine 2\nLine 3\nLine 4\nLine 5\nLine 6\nLine 7\nLine 8\nLine 9';
            const filePath = await createTestFile('test.txt', content);
            const result = await fileEditor.invoke({ command: 'insert', path: filePath, insert_line: 5, new_str: 'INSERTED' }, context);
        ...
        it('handles multi-line insertion', async () => {
            const filePath = await createTestFile('test.txt', 'Line 1\nLine 2');
            const result = await fileEditor.invoke({ command: 'insert', path: filePath, insert_line: 1, new_str: 'NEW 1\nNEW 2\nNEW 3' }, context);
            expect(result).toContain('has been edited');
            const fileContent = await fs.readFile(filePath, 'utf-8');
            expect(fileContent).toBe('Line 1\nNEW 1\nNEW 2\nNEW 3\nLine 2');
        });
        it('handles insertion in empty file', async () => {
            const filePath = await createTestFile('empty.txt', '');
            const result = await fileEditor.invoke({ command: 'insert', path: filePath, insert_line: 0, new_str: 'First line' }, context);
            expect(result).toContain('has been edited');
            const fileContent = await fs.readFile(filePath, 'utf-8');
        ...
        describe('error cases', () => {
            it('throws when insert_line is negative', async () => {
                const filePath = await createTestFile('test.txt', 'Line 1\nLine 2');
                await expect(fileEditor.invoke({ command: 'insert', path: filePath, insert_line: -1, new_str: 'NEW' }, context)).rejects.toThrow('insert_line');
            });
            it('throws when insert_line is beyond file length', async () => {
                const filePath = await createTestFile('test.txt', 'Line 1\nLine 2');
                await expect(fileEditor.invoke({ command: 'insert', path: filePath, insert_line: 10, new_str: 'NEW' }, context)).rejects.toThrow('insert_line');
            });
            it('throws when file not found', async () => {
                const nonExistentPath = path.join(testDir, 'nonexistent.txt');
                await expect(fileEditor.invoke({ command: 'insert', path: nonExistentPath, insert_line: 0, new_str: 'NEW' }, context)).rejects.toThrow('does not exist');
            });
            it('throws when path is directory', async () => {
                const dirPath = await createTestDirectory('testdir', {});
                await expect(fileEditor.invoke({ command: 'insert', path: dirPath, insert_line: 0, new_str: 'NEW' }, context)).rejects.toThrow('directory');
            });
        });
    });
...
//# sourceMappingURL=file-editor.test.js.map